...
const { waitTimeout, counterNotNull, processProfileDataFromSearch, } = require('./data-processors/data-processors');
...
app.get('/vkget/:ida', (req, res) => {
    ...
    const getInfoByUserId = async (id) => {
        const res = await fetch(`${_apibase}users.get?user_ids=${id}&fields=sex,bdate,sex,city,contacts,country,exports,followers_count,has_photo,photo_max,photo_max_orig,photo_100,online,exports,counters,relation,last_seen&access_token=${access_token_actual}&v=5.103`);
        if (!res.ok) {
        ...
        try {
            ...
            for (let i = 0; i < profiles.length; i++) {
                try {
                    ...
                    await db.insertUpdateProfile(profileMapperTs(flatten(profiles[i].response[0])));
...
app.get('/matchfromsearch/:query/:qnt/:offset/:f/:t/:city/:year/:month/:day', (req, res) => {
...
app.get('/insertcheck/:profileId/:firstName/:lastName/:pickedYear', (req, res) => {
    ...
    (async (profileId, firstName, lastName, pickedYear) => {
        try {
            let result = await db.insertUpdateProfileCheck(profileId, firstName, lastName, pickedYear);
            res.send(result);
        }
        catch (e) {
            res.send(e);
...
app.get('/dbmanager/insertchecksingle/:profileId/:firstName/:lastName', (req, res) => {
    const { profileId, firstName, lastName } = req.params;
    ...
    (async (profileId, firstName, lastName) => {
        try {
            let result = await db.insertUpdateProfileCheckSingle(profileId, firstName, lastName);
...
app.get('/searchtest', (req, res) => {
    ...
    const searhUsersTest = async () => {
        const res = await fetch(`${_apibase}users.search?sort=1&offset=0&count=100&fields=sex,bdate,city,home_town,has_photo,counters,followers_count&city=1&sex=1&status=1&age_from=23&age_to=30&has_photo=1&can_access_closed=true&access_token=${access_token_actual}&v=5.103`);
        if (!res.ok) {
            throw new Error(`Something was wrong`);
        }
        const data = await res.json();
...
app.ws('/test', (ws, req) => {
    const getUniqueId = () => {
        const s4 = () => Math.floor((1 + Math.random()) * 0x100000)
            .toString(16)
            .substring(1);
        return s4() + s4() + '-' + s4();
    };
